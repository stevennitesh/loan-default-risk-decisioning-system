...
def run_feature_build(config_path: str | Path = "configs/base.yaml") -> list[dict[str, Any]]:
    ...
    with duckdb.connect(str(duckdb_path)) as connection:
        _ensure_staging_tables(connection)
        for sql_file in FEATURE_SQL_FILES:
...
def _ensure_staging_tables(connection: duckdb.DuckDBPyConnection) -> None:
    existing_tables = {row[0] for row in connection.execute("SHOW TABLES").fetchall()}
    missing_tables = sorted(set(REQUIRED_STAGING_TABLES).difference(existing_tables))
    if missing_tables:
        raise FeatureBuildError(f"Missing required staging tables: {', '.join(missing_tables)}")


def _profile_feature_tables(connection: duckdb.DuckDBPyConnection) -> list[dict[str, Any]]:
